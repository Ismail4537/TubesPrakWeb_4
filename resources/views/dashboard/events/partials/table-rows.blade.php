@forelse ($events as $index => $event)
    <tr class="hover:bg-gray-50 transition">
        ...
        <td class="px-6 py-4 font-medium text-gray-900">{{ $event->judul }}</td>
        <td class="px-6 py-4 text-gray-600">{{ $event->category->name ?? '-' }}</td>
        <td class="px-6 py-4">
            <span class="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-full text-[10px] font-bold uppercase">
                {{ $event->status ?? 'Draft' }}
            </span>
        </td>
