        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="relative w-full h-64 border-2 border-gray-300 border-dashed rounded-xl overflow-hidden bg-gray-50 group hover:border-blue-400 transition">
                        ...
                        <label for="image_upload" class="cursor-pointer">
                            <div id="placeholder-upload" class="absolute inset-0 flex flex-col items-center justify-center p-6 text-center transition-opacity duration-300 {{ isset($event['foto']) ? 'hidden' : '' }}">
                            ...
                            <img id="preview-image" 
                                 class="absolute inset-0 w-full h-full object-cover {{ isset($event['foto']) ? '' : 'hidden' }} z-10" 
                                 src="{{ isset($event['foto']) ? asset('storage/' . $event['foto']) : '#' }}" 
                                 alt="Preview" />

                            <div id="hover-overlay" class="absolute inset-0 bg-black/40 {{ isset($event['foto']) ? 'flex' : 'hidden' }} z-20 items-center justify-center transition opacity-0 hover:opacity-100">
                                <span class="text-white text-xs font-semibold bg-black/50 px-4 py-2 rounded-full backdrop-blur-sm">Ganti Foto</span>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="judul" id="title" required value="{{ old('judul', $event['judul'] ?? '') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="kategori" id="kategori" required
                            ...
                            <option value="expo" {{ old('kategori', $event['kategori'] ?? '') == 'expo' ? 'selected' : '' }}>Expo</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga" id="price" required value="{{ old('harga', $event['harga'] ?? '') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="lokasi" id="location" required value="{{ old('lokasi', $event['lokasi'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm shadow-sm transition">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="tanggal" id="date" required 
                            value="{{ old('tanggal', isset($event['tanggal']) ? date('Y-m-d\TH:i', strtotime($event['tanggal'])) : '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm shadow-sm transition">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="description" rows="4" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm shadow-sm transition">{{ old('deskripsi', $event['deskripsi'] ?? '') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="alamat_lengkap" id="address" rows="2" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm shadow-sm transition">{{ old('alamat_lengkap', $event['alamat_lengkap'] ?? '') }}</textarea>
                </div>
            </div>

            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                <a href="{{ route('dashboard.events.index') }}" class="px-5 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 transition shadow-sm">
                    Batal
                </a>
