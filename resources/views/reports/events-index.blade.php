...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3">
                <!-- Button View PDF -->
                <a href="{{ route('reports.events.pdf.view') }}" target="_blank" 
                ...
                <!-- Button Download PDF -->
                <a href="{{ route('reports.events.pdf.download') }}" 
                   class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ $events->count() }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
            <p class="text-sm text-gray-500 uppercase tracking-wide">Scheduled</p>
            <p class="text-2xl font-bold text-blue-600 mt-1">{{ $events->where('status', 'scheduled')->count() }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
            <p class="text-sm text-gray-500 uppercase tracking-wide">Ongoing</p>
            <p class="text-2xl font-bold text-green-600 mt-1">{{ $events->where('status', 'ongoing')->count() }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
            <p class="text-sm text-gray-500 uppercase tracking-wide">Completed</p>
            <p class="text-2xl font-bold text-purple-600 mt-1">{{ $events->where('status', 'completed')->count() }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
            <p class="text-sm text-gray-500 uppercase tracking-wide">Cancelled</p>
            <p class="text-2xl font-bold text-red-600 mt-1">{{ $events->where('status', 'cancelled')->count() }}</p>
        </div>
    </div>
