...
<x-slot:title> {{$title}} </x-slot:title>
    ...
    <div>
        ...
        <div class="p-4 grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4"> 
            @foreach ( $highlightEvents as $event )
                <a :class="activeTab === 'Sorotan' ? '' : 'hidden'" href="{{ route('event.show', ['slug' => $event->slug]) }}" class="my-4 shrink-0 w-full">
            ...
            @foreach ( $popularEvents as $event )
                <a :class="activeTab === 'Populer' ? '' : 'hidden'" href="{{ route('event.show', ['slug' => $event->slug]) }}" class="my-4 shrink-0 w-full">
                <div class="border border-gray-300 w-full max-w-xs bg-white rounded-xl shadow-lg overflow-hidden h-full flex flex-col">    
                    {{-- Gambar --}}
                    <div class="relative h-32 sm:h-42"> 
                        <img src="{{ asset('../Image/Preview.jpg') }}" class="w-full h-full object-cover rounded-t-xl">
            ...
            @foreach ( $latestEvents as $event )
                <a :class="activeTab === 'Terbaru' ? '' : 'hidden'" href="{{ route('event.show', ['slug' => $event->slug]) }}" class="my-4 shrink-0 w-full">
                <div class="border border-gray-300 w-full max-w-xs bg-white rounded-xl shadow-lg overflow-hidden h-full flex flex-col">    
                    {{-- Gambar --}}
                    <div class="relative h-32 sm:h-42"> 
                        <img src="{{ asset('../Image/Preview.jpg') }}" class="w-full h-full object-cover rounded-t-xl">
            ...
            @foreach ( $freeEvents as $event )
                <a :class="activeTab === 'Gratis' ? '' : 'hidden'" href="{{ route('event.show', ['slug' => $event->slug]) }}" class="my-4 shrink-0 w-full">
                <div class="border border-gray-300 w-full max-w-xs bg-white rounded-xl shadow-lg overflow-hidden h-full flex flex-col">    
                    {{-- Gambar --}}
                    <div class="relative h-32 sm:h-42"> 
                        <img src="{{ asset('../Image/Preview.jpg') }}" class="w-full h-full object-cover rounded-t-xl">
    ...
    <div class="px-4 mb-10">
        ...
        <div class="px-2 grid grid-cols-[repeat(auto-fit,minmax(177px,1fr))]">
            ...
            <a href="{{ route('contac.show', ['id' => $user->id]) }}" class="my-4 shrink-0 w-48 ">
                <div class="p-2 flex flex-col items-center w-full max-w-xs rounded-xl overflow-hidden h-full">
                    <img src="{{ asset($user->profile_photo_path ?? 'https://ui-avatars.com/api/?name='.$user->name) }}" alt="gambar" class="w-35 h-35 rounded-full object-cover mb-4">
                    <h2 class="text-md font-medium text-gray-900 text-center">{{ $user->name }}</h2>
                </div>
            </a>
            @endforeach
        </div>
