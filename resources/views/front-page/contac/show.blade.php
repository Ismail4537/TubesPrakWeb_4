            ...
            <div class="md:col-span-1 space-y-6">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 text-center">
                    <div class="relative mx-auto w-32 h-32 mb-4">
                        <img src=@if ($user->profile_photo_path != null) {{ asset('storage/' . $user->profile_photo_path) }}
                        @else
                            "https://ui-avatars.com/api/?name={{ urlencode($user->name ?? 'User') }}&background=random&color=fff&size=128" @endif
                    ...
                    @foreach ($listEvent as $event)
                        <a href={{ route('event.show', ['slug' => $event['slug']]) }}
                            ...
                            <img class="w-full h-40 object-cover"
                                src=@if ($event['image_path'] != null) {{ asset($event['image_path']) }}
                            @else
                                {{ asset('Image/Preview.jpg') }} @endif
                                alt="Gambar Event">
                            <div class="p-5">
                                <h4
                                ...
                                <div class="text-sm text-slate-500 space-y-2">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="text-xl font-semibold text-gray-900 mb-3">{{ $event['title'] }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-indigo-400">📅</span> <span>{{ $event['start_date_time'] }} /
                                            {{ $event['end_date_time'] }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-indigo-400">📍</span> <span
                                            class="truncate">{{ $event['location'] }}</span>
                                    </div>
                                </div>
                            </div>
                        </a>
